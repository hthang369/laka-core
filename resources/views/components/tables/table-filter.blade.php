<td {{$attributes->merge(['scope' => 'col'])}}>
    ...
    @else
        @if (str_is($field->dataType, 'date'))
            <x-dynamic-component :component="$type" :name="picker"/>
        @elseif (str_is($type, 'select'))
            {!! Form::select('filter_lookup', $field->lookup->items, null, ['class' => 'form-control form-control-sm', 'placeholder' => translate('table.select')]) !!}
        @else
